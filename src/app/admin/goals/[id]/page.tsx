...
import AdminGoalDetails from '../components/AdminGoalDetails';
import GoalDetailsLoader from '../components/GoalDetailsLoader';

export default async function AdminGoalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [goalRes, tasksRes] = await Promise.all([
      fetch(`${baseUrl}/api/goals/${resolvedParams.id}`, {
        headers: authToken ? { Cookie: `authToken=${authToken}` } : undefined,
        cache: 'no-store'
      }),
      fetch(`${baseUrl}/api/tasks?goalId=${resolvedParams.id}`, {
        headers: authToken ? { Cookie: `authToken=${authToken}` } : undefined,
        cache: 'no-store'
      })
    ]);
    ...
    if (!goalRes.ok) {
      ...
      return <GoalDetailsLoader goalId={Number(resolvedParams.id)} />;
    }
