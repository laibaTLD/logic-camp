...
async function testTaskGoalModels() {
  try {
    ...
    // Access goal and project through the association properties
    const goalData = taskWithAssociations.get('goal');
    console.log('- Goal title:', goalData ? goalData.title : 'No goal');
    
    const projectData = goalData ? goalData.get('project') : null;
    console.log('- Project name:', projectData ? projectData.name : 'No project');
