...
class ProjectMessage extends Model<ProjectMessageAttributes, ProjectMessageCreationAttributes> implements ProjectMessageAttributes {
  ...
  async notifyProjectMembers() {
    try {
      const { Project, ProjectMember, User } = require('./index');
      ...
      // Notify each member (except sender) about the new message
      for (const member of projectMembers) {
        if (member.userId !== this.senderId) {
          await notifyUnreadMessages(member.userId, 1, project.name, this.projectId);
        }
      }
    } catch (error) {
