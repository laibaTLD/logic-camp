  ...
  async notifyTaskChanges(previousValues?: Partial<TaskAttributes>) {
    try {
      ...
      if (this.assignedToId && (!previousValues || previousValues.assignedToId !== this.assignedToId)) {
        await notifyTaskAssigned(this.id.toString(), this.assignedToId.toString(), this.createdById.toString(), this.title);
      }
      
      // Check if task was completed
      if (this.status === 'completed' && previousValues && previousValues.status !== 'completed') {
        await notifyTaskCompleted([this.id.toString()], this.createdById.toString(), this.assignedToId?.toString(), this.title);
      }
    } catch (error) {
    ...
    },
    {
      ...
      hooks: {
        ...
        afterUpdate: async (task: Task) => {
          if (task.changed()) {
            const previousValues = task._previousDataValues as Partial<TaskAttributes>;
