...
class Task extends Model<TaskAttributes, TaskCreationAttributes> implements TaskAttributes {
  ...
  async notifyTaskChanges(previousValues?: Partial<TaskAttributes>) {
    try {
      ...
      const currentAssigneeIds = currentAssignees.map((ta: any) => ta.userId);
      
      // Check if task has new assignees (for new tasks or updated assignments)
      if (currentAssigneeIds.length > 0) {
        ...
        if (project && project.team && project.team.members) {
          ...
          for (const assigneeId of currentAssigneeIds) {
            await notifyTaskAssignedToTeam(teamMemberIds, assigneeId, this.title, project.name, this.id);
          }
        } else {
          // Fallback to individual notifications
          for (const assigneeId of currentAssigneeIds) {
            await notifyTaskAssigned(assigneeId, this.title, 'Project', this.id);
          }
        }
      }
      ...
      if (this.status === 'completed' && previousValues && previousValues.status !== 'completed') {
        const userIds = currentAssigneeIds.length > 0 ? [this.createdById, ...currentAssigneeIds] : [this.createdById];
