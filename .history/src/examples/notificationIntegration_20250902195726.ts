...
async function getTeamMembers(teamId: number): Promise<{ userId: number; role?: string }[]> {
  // This would query your TeamMembers table
  // Return array of { userId: number, role?: string }
  return [];
}

async function getProject(projectId: number): Promise<{ id: number; name: string; teamId: number }> {
  // This would query your Projects table
  // Return { id: number, name: string, teamId: number }
  return { id: projectId, name: 'Sample Project', teamId: 1 };
}

async function getTeam(teamId: number): Promise<{ id: number; name: string }> {
  // This would query your Teams table
  // Return { id: number, name: string }
  return { id: teamId, name: 'Sample Team' };
}

async function findExistingChatNotification(userId: number, projectId: number): Promise<{ id: number; message: string } | null> {
