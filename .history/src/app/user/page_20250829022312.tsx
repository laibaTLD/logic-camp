...
import ProjectsGrid from "../admin/components/ProjectsGrid"; // Reuse if possible, or create user-specific
import Header from "../admin/components/Header"; // Reuse header
import useUserData from "./hooks/useUserData"; // Will create this hook later
...
export default function UserPanel() {
  ...
  const {
    projects,
    loadingProjects,
  } = useUserData(); // Custom hook for user data
        ...
        <div className="flex flex-col gap-8 animate-fadeInUp">
          ...
          <section className="flex-1 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-6 shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
            ...
            {loadingProjects ? (
              ...
              <ProjectsGrid projects={projects} /> // Adapt for user projects
            )}
          </section>
        </div>
      </main>
