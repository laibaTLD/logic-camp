...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await team.update(parsed.data as any); // cast to any to avoid TS errors
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Assign users safely for Postgres
    const userIds = parsed.data.userIds;
    for (const userId of userIds) {
      await TeamMember.findOrCreate({
        where: { team_id: teamId, user_id: userId }, // match DB column names
        defaults: { team_id: teamId, user_id: userId },
      });
    }
