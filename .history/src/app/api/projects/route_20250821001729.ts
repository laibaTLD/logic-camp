...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Project || !ProjectMember) {
      return NextResponse.json({ error: "Models missing" }, { status: 500 });
    }
    ...
    if (!payload.userId) {
      return NextResponse.json({ error: "Invalid user payload" }, { status: 401 });
    }
    ...
    if (Array.isArray(memberIds) && memberIds.length > 0) {
      await Promise.all(
        memberIds.map((id: number) =>
          ProjectMember.create({ projectId: project.id, userId: id })
        )
      );
    }
    ...
    return NextResponse.json(
      { success: true, message: "Project created successfully", project: createdProject },
      { status: 201 }
    );
  } catch (error: any) {
