...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Project || !TeamMember) {
      return NextResponse.json({ error: 'Models not found. Check getModels()' }, { status: 500 });
    }
    ...
    // Add team members (DB fields: project_id, user_id)
    if (Array.isArray(memberIds) && memberIds.length > 0) {
      await Promise.all(
        memberIds.map((id: number) =>
          TeamMember.create({ project_id: project.id, user_id: id })
        )
      );
    }
