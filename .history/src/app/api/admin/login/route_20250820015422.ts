...
if(!admin.id || !admin.email || !admin.role) {
  console.log("Payload not found while creating token")
  return NextResponse.json({message:"Payload not found while creating token"})
}

const token = jwt.sign(
  { id: admin.id, email: admin.email, role: admin.role },
  JWT_SECRET as string, // assure TypeScript it’s a string
  { expiresIn: "8h" }
);
