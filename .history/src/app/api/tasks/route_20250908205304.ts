...
export async function POST(req: NextRequest) {
  try {
    const { Task, User, Goal, sequelize } = await getModels();
    ...
    const result = await sequelize.transaction(async (t) => {
      const task = await Task.create({
        ...
        assigned_to_id: assigneeIds.length > 0 ? assigneeIds[0] : undefined, // Keep first assignee for backward compatibility
