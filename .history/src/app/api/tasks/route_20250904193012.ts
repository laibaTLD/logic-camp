...
export async function GET(req: NextRequest) {
  try {
    ...
    if (projectId) {
      ...
      whereClause.projectId = projectIdNum;
    }
...
export async function POST(req: NextRequest) {
  try {
    const { Task, User, Project, TaskAssignee, sequelize } = await getModels();
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    await task.update({
      ...
      assignedToId: (validatedData.assigneeIds !== undefined || validatedData.assignedToId !== undefined) ? (assigneeIds.length > 0 ? assigneeIds[0] : undefined)
