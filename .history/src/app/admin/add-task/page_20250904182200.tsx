                ...
                <div className="relative">
                  <div className="w-full px-6 py-4 bg-gray-800/50 border border-white/10 rounded-2xl backdrop-blur-sm relative z-10 max-h-48 overflow-y-auto">
                    ...
                    {teamMembers.map((member) => (
                      <div key={member.id} className="flex items-center gap-3 py-2 px-2 hover:bg-white/5 rounded-lg cursor-pointer transition-colors" onClick={() => handleAssigneeToggle(member.id)}>
                        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                        ...
                        <span className="text-gray-400 text-sm ml-auto">{member.email}</span>
                      </div>
                    ))}
