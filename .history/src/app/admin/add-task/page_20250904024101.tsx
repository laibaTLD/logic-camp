...
export default function AddTaskPage() {
  ...
  const handleSelectAllAssignees = () => {
    ...
    setFormData(prev => ({
      ...prev,
      assigneeIds: prev.assigneeIds.length === allUserIds.length ? [] : allUserIds
    }));
  };
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-8 shadow-[0_10px_40px_rgba(0,0,0,0.35)] space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <div className="w-full px-6 py-4 bg-gray-800/50 border border-white/10 rounded-2xl backdrop-blur-sm relative z-10 max-h-48 overflow-y-auto">
                    {/* Select All Option */}
                    <div className="flex items-center gap-3 py-2 px-2 hover:bg-white/5 rounded-lg cursor-pointer transition-colors" onClick={handleSelectAllAssignees}>
                      <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                        formData.assigneeIds.length === users.length
                        ...
                        {formData.assigneeIds.length > 0 && formData.assigneeIds.length < users.length && (
                    ...
                    {users.map((user) => (
                      <div key={user.id} className="flex items-center gap-3 py-2 px-2 hover:bg-white/5 rounded-lg cursor-pointer transition-colors" onClick={() => handleAssigneeToggle(user.id)}>
                        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                        ...
                        <span className="text-gray-400 text-sm ml-auto">{user.email}</span>
                      </div>
                    ))}
