...
export default function UserTable() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.sort((a, b) => {
      const av: any =
        sortKey === "status" ? a.isApproved : (a as any)[sortKey];
      const bv: any =
        sortKey === "status" ? b.isApproved : (b as any)[sortKey];
        ...
        <AnimatePresence initial={false}>
          {loadingUsers ? (
            [...Array(5)].map((_, i) => (
              <motion.div
                key={`s-${i}`}
                className="p-4 grid grid-cols-12 gap-2 animate-pulse"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              >
                ...
                <div className="col-span-2 h-8 bg-white/10 rounded" />
              </motion.div>
            ))
          ) : filtered.length ? (
