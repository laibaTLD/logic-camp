...
export default function NewProjectModal({ isOpen, onClose }: NewProjectModalProps) {
  ...
  const [status, setStatus] = useState<'todo' | 'inProgress' | 'testing' | 'completed' | 'archived'>('todo');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate dates order when both provided
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-fadeIn" onClick={onClose}>
      ...
      >
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          {/* Status Selector */}
          <div className="space-y-2">
            <label className="text-gray-300 text-sm font-medium block">Status</label>
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as any)}
              className="w-full rounded-xl bg-gray-800/50 border border-white/20 px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 text-base"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-gray-300 text-sm font-medium block">Start Date</label>
              <input
                type="date"
                value={startDate}
            ...
            <div className="space-y-2">
              <label className="text-gray-300 text-sm font-medium block">End Date</label>
              <input
                type="date"
                value={endDate}
