        ...
        <div className="absolute top-6 right-6">
          {project.status && (
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full shadow-lg backdrop-blur-sm
              ${project.status === 'completed' ? 'bg-green-500/15 border border-green-500/40' : 
                project.status === 'in-progress' ? 'bg-blue-500/15 border border-blue-500/40' : 
                project.status === 'review' ? 'bg-purple-500/15 border border-purple-500/40' : 
                'bg-gray-500/15 border border-gray-500/40'}
              hover:shadow-lg hover:scale-105 transition-all duration-300 group-hover:shadow-md
              ${project.status === 'completed' ? 'group-hover:shadow-green-500/20' : 
                project.status === 'in-progress' ? 'group-hover:shadow-blue-500/20' : 
                project.status === 'review' ? 'group-hover:shadow-purple-500/20' : 
                'group-hover:shadow-gray-500/20'}`}>
              <CheckCircle className={`h-3.5 w-3.5 ${getStatusColor(project.status)} animate-pulse`} />
