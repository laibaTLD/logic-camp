...
export default function NewProjectModal({ isOpen, onClose, users }: NewProjectModalProps) {
  ...
  const [selectedTeam, setSelectedTeam] = useState<number | undefined>(undefined); // ✅ no null, only undefined
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createProject({
        name,
        description,
        teamId: selectedTeam, // ✅ already number | undefined
      ...
      setDescription("");
      setSelectedMembers([]);
      setSelectedTeam(undefined); // ✅ reset properly
      onClose();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-2xl p-6 w-full max-w-md shadow-[0_12px_40px_rgba(0,0,0,0.35)] relative">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          {/* ✅ Team Selector */}
          <div>
            <label className="text-gray-300 text-sm mb-1 block">Assign Team</label>
            <select
              value={selectedTeam ?? ""} // fallback to empty string
              onChange={(e) =>
                setSelectedTeam(e.target.value ? Number(e.target.value) : undefined)
              }
              className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
            >
              <option value="">-- Select Team --</option>
              {/* TODO: replace with real teams fetched from DB */}
