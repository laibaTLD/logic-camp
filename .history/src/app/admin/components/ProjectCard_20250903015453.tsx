...
import { Users, Calendar, FolderKanban, Edit, Trash2, ChevronDown, ChevronUp } from "lucide-react";
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, onOpenProject, onEditTask, onDeleteTask, tasks = [], onLoadTasks }) => {
  const [showTasks, setShowTasks] = useState(false);
  const [tasksLoaded, setTasksLoaded] = useState(false);

  const handleToggleTasks = () => {
    if (!tasksLoaded && onLoadTasks) {
      onLoadTasks();
      setTasksLoaded(true);
    }
