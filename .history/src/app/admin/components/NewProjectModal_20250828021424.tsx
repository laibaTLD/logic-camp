...
interface NewProjectModalProps {
  ...
  teams: { id: number; name: string }[]; // ✅ added teams
}

export default function NewProjectModal({ isOpen, onClose, users, teams }: NewProjectModalProps) {
  const { createProject } = useAdminData();

  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [selectedMembers, setSelectedMembers] = useState<number[]>([]);
  const [selectedTeamId, setSelectedTeamId] = useState<number | null>(null); // ✅ team state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createProject({
        name,
        description,
        teamId: selectedTeamId, // ✅ actual team
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-2xl p-6 w-full max-w-md shadow-[0_12px_40px_rgba(0,0,0,0.35)] relative">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          {/* ✅ Team Dropdown */}
          <div>
            <label className="text-gray-300 text-sm mb-1 block">Select Team</label>
            <select
          ...
          {/* Members selection */}
          <div>
            <label className="text-gray-300 text-sm mb-1 block">Project Members</label>
            <select
              multiple
