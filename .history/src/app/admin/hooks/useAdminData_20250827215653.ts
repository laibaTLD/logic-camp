...
export default function useAdminData() {
  ...
  const openProject = useCallback((id: number) => {
    ...
    // later: router.push(`/admin/projects/${id}`);
  }, []);

  const editProject = useCallback(async (id: number, updates: Partial<Project>) => {
    try {
  ...
  const addTaskToProject = useCallback(async (id: number, task: { title: string }) => {
    try {
  ...
  return {
    ...
    projects,
    loadingProjects,
    openProject,      // ✅ now properly declared
    editProject,
    deleteProject,
    addTaskToProject,
  };
