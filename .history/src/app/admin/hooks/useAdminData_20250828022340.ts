...
export default function useAdminData() {
  ...
  const createTeam = useCallback(
    async (team: { name: string; members: number[] }) => {
      try {
        ...
        const result = await res.json();
        const newTeam: Team = result.team || result;
  ...
  const createProject = useCallback(
    async (project: {
      ...
      startDate?: string;
      endDate?: string;
      teamId?: number; // optional now, can be undefined
      members?: number[];
    }) => {
      try {
        // Ensure teamId is undefined if null
        const teamIdSafe = project.teamId ?? undefined;
