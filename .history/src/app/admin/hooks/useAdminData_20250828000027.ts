...
const createProject = useCallback(
  async (project: {
    ...
    endDate?: string;
    teamId?: number;
    members?: number[]; // optional, backend can handle this later
  }) => {
    try {
      // Only send allowed fields to backend; no createdById
      const payload: any = {
        name: project.name,
      ...
      if (project.members) payload.members = project.members; // optional, backend can save members later
