...
export default function useAdminData() {
  ...
  useEffect(() => {
    async function fetchTeams() {
      try {
        const headers = getAuthHeaders();
        const res = await fetch("/api/teams", { headers, credentials: 'include' });
        if (!res.ok) throw new Error(`Error ${res.status}`);
        const data = await res.json();
        // fallback to empty array if API returns nothing
    ...
    async (id: number, updates: Partial<User>) => {
      try {
        ...
        if (updates.role && !allowedRoles.includes(updates.role)) {
          console.error("Invalid role value:", updates.role, "Expected one of:", allowedRoles);
          throw new Error(`Invalid role value: ${updates.role}. Expected one of: ${allowedRoles.join(", ")}`);
        }
