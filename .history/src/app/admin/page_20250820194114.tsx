...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setUsers(data.users || data); // works with {users: [...]} or [...]
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ padding: "2rem" }}>
      ...
      <button
        ...
        style={{ marginBottom: "1rem", background: "red", color: "white", padding: "0.5rem 1rem", border: "none", borderRadius: "5px" }}
      >
        Logout
      </button>
              ...
              <tr key={user.id}>
                ...
                <td>
                  {editingUser?.id === user.id ? (
                    ...
                    <>
                      {!user.isApproved && <button onClick={() => approveUser(user.id)} style={{ marginRight: "0.5rem" }}>Approve</button>}
                      {user.isApproved && <button onClick={() => rejectUser(user.id)} style={{ background: "orange", marginRight: "0.5rem" }}>Reject</button>}
                      <button onClick={() => startEdit(user)} style={{ marginRight: "0.5rem" }}>Edit</button>
